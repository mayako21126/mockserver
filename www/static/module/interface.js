...
window.interface = {
     ...
     */
    setMem: function() {
        $('#setmem').click(function() {
            $.getJSON('/interface/setmem', {}).done(function(data) {
                alert(data.data)
            }).fail(function(err) {
                alert(err.message)
            });
    ...
    proxyBind: function() {
        ...
        $('.oper_proxy').click(function() {
            var $this = $(this),
                is_proxy = $(this).attr('data-is-proxy'),
                mockid = $(this).attr('mockid'),
                txt = is_proxy === "0" ? _this.opts.LN.interface.index.closeProxy : _this.opts.LN.interface.index.openProxy,
